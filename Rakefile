...
task :update_podspec do
  files = %Q<'#{public_header_files_pattern}'>
  podspec = File.read('FactoryKit.podspec')
  podspec.sub!(/(s\.public_header_files\s*=\s*).+/, "\\1#{files}")
  File.write('FactoryKit.podspec', podspec)
end

desc "Releases a new version of the Pod"
task :release => :update_podspec do
...
def public_header_files_pattern
  files = ['FactoryKit.h']
  open('FactoryKit/FactoryKit.h').each do |line|
    if line =~ /BEGIN/ .. line =~ /END/
      line.match(/^\s*#\s*import\s+"([^"]+)"/) do |m|
        files << m[1]
      end
    end
  end
  "FactoryKit/{" + files.join(',') + "}"
end
